...
public class IcebergOptionsTest {
    ...
    @Test
    public void testWhitespaceAroundSemicolonsIsTrimmed() {
        Options options = new Options();
        options.set(IcebergOptions.METASTORE_DATABASE, " db1 ; db2 ; db3 ");
    ...
    @Test
    public void testSingleDatabaseNoSemicolon() {
        Options options = new Options();
        options.set(IcebergOptions.METASTORE_DATABASE, "only_db");

        List<String> databases = IcebergOptions.metastoreDatabases(options, FALLBACK_DB);

        assertThat(databases).containsExactly("only_db");
    }
}
